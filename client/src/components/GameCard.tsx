...
const GameCard = ({ game, toggleFavorite, isFavorite }: GameCardProps) => {
    ...
    return (
        <Link to={`/game/${game.id}`} className="game-card"> 
                <button className="game-card__like-btn" onClick={handleLikeClick}>{isFavorite ? <FaStar/> : <FaRegStar/>}</button>
                <img
                ...
                <div className='game-card__genre-container'> 
                    {game.genres.map(genre => (
                        <span className='game-card__genre' key={genre.id}>{genre.name}</span>
                    ))}
                </div>
        </Link>
    );
