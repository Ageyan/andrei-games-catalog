            ...
            <div className="home-page__container">
                <div className="home-page__genres-search">
                    {genres.map(g => (
                        ...
                        >
                            <img
                                className="home-page__genre-img"
                                alt={g.alt}
                                src={g.src}
                            />
                ...
                <div className="home-page__main-layout">
                    <aside>
                        <div className="home-page__sort">
                            <div
                                className="home-page__sort-container"
                                onClick={e => {
                                    setIsOpen(!isOpen);
                                    e.stopPropagation();
                                }}
                            >
                                <span>
                                    {sorts.find(s => s.sort === sortOrder)
                                        ?.name || sorts[0].name}
                                </span>
                                <IoIosArrowDown
                                    className={`home-page__sort-icon ${isOpen ? 'open' : ''}`}
                                />
                            </div>
                            {isOpen && (
                                <div className="home-page__sort-options">
                                    {sorts.map(s => (
                                        <div
                                            key={s.id}
                                            className={`home-page__sort-option ${sortOrder === s.sort ? 'selected' : ''}`}
                                            onClick={() => {
                                                setPage(1);
                                                setSortOrder(s.sort);
                                                setIsOpen(false);
                    ...
                    <div className="home-page__main-content">
                        {loader && (
                            <p className="loader-text">Game list update...</p>
                        )}
                        {!loader && games.length === 0 && (
                            <p className="loader-text">Games not found...</p>
                        )}
